...
use crate::{
    plan::{ConcreteField, ConcreteSelectionSetId, PlanBoundaryId, RuntimeConcreteSelectionSet},
...
pub(crate) struct ConcreteCollectionSetSeed<'ctx, 'parent> {
    ...
    pub fields: &'parent [ConcreteField],
    pub typename_fields: &'parent [ResponseEdge],
}

impl<'ctx, 'parent> ConcreteCollectionSetSeed<'ctx, 'parent> {
    pub fn new_from_id(ctx: &'parent SeedContextInner<'ctx>, id: ConcreteSelectionSetId) -> Self {
    ...
    pub fn new(ctx: &'parent SeedContextInner<'ctx>, selection_set: &'parent RuntimeConcreteSelectionSet) -> Self {
        Self {
            ctx,
