...
use crate::{plan::CollectedSelectionSet, request::SelectionSetType, response::ResponseValue};
...
impl<'de, 'ctx, 'parent> DeserializeSeed<'de> for SelectionSetSeed<'ctx, 'parent> {
        ...
        D: serde::Deserializer<'de>,
    {
        match self.collected {
            &CollectedSelectionSet::Concrete(id) => {
                ConcreteCollectionSetSeed::new_from_id(self.ctx, id).deserialize(deserializer)
            }
            ...
            .deserialize(deserializer),
            CollectedSelectionSet::RuntimeConcrete(selection_set) => {
                ConcreteCollectionSetSeed::new(self.ctx, selection_set).deserialize(deserializer)
            }
        }
    }
}
