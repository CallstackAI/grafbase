...
impl ProductionServer {
    pub async fn build(
        ...
        if let Some(path) = federated_graph_schema_path {
            let sdl = tokio::fs::read_to_string(&path)
                .await
                .map_err(|err| ServerError::InvalidFederatedGraphSdl(err.to_string()))?;
            let federated_graph = graphql_federated_graph::from_sdl(&sdl).map_err(|err| {
                ServerError::InvalidFederatedGraphSdl(format!("Invalid federated graph SDL: {}", err))
            })?;
            let config = engine_v2::VersionedConfig::from_graph(federated_graph);
            return Ok(Self::FederatedFromStaticConfig { message_sender, config });
        }
        ...
        if let Some(config) = federated_graph_config {
            Ok(Self::FederatedFromTsConfig { config, message_sender })
        } else {
    ...
    pub async fn serve(self, listen_address: IpAddr, port: u16) -> Result<(), ServerError> {
        match self {
            ProductionServer::FederatedFromStaticConfig { message_sender, config } => {
                let _ = message_sender.send(ServerMessage::Ready {
                    listen_address,
                    port,
                    is_federated: true,
                });
                federated_dev::static_graph_run(port, true, config)
                    .await
                    .map_err(|error| ServerError::GatewayError(error.to_string()))
            }
            ProductionServer::FederatedFromTsConfig { config, message_sender } => {
                let _ = message_sender.send(ServerMessage::Ready {
                    listen_address,
                    port,
                    is_federated: true,
                });
                federated_dev::run(port, true, constant_watch_receiver(config))
                    .await
                    .map_err(|error| ServerError::GatewayError(error.to_string()))
            }
            ProductionServer::V1 {
